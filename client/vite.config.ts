...
const TARGET_PROXY = {
    ...
    '/science-portal/oidc-callback': {
      ...
      configure: (proxy, options) => {
        proxy.on('proxyRes', function (proxyRes, req, res) {
          if (proxyRes.headers.location?.includes('haproxy.cadc.dao.nrc.ca')) {
  ...
  [DEV_MODE_LOCAL]: {
    ...
    '/science-portal/oidc-callback': {
      target: 'https://localhost:5173/science-portal',
      changeOrigin: true,
      secure: false,
    },
  },
  ...
  ({ command }) =>
    ({
      ...
      server: {
        ...
        port: 5173,
        proxy: TARGET_PROXY[CURRENT_DEV_MODE],
